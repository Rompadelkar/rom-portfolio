...
export function About() {
  const [tab, setTab] = useState<"experience" | "education" | "certs">(
    "experience",
  );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-8 space-y-4">
              ...
              <p className="text-slate-300 text-base sm:text-lg leading-relaxed font-normal">
                I have Completed BTECH In Computer Engineering undergraduate at
                Shah And Anchor Kutchhi Engineering College, Mumbai (CGPA:
                7.4/10). During my internship at{" "}
                <strong className="text-blue-400">
                  QualityKiosk Technologies
                </strong>{" "}
                within the Digital Experience Monitoring (DEM) team, I
                engineered and validated AI Buddy solutions that synthesize
                complex telemetry logs into actionable root-cause incident
                summaries for SRE teams.
              </p>
              <p className="text-slate-300 text-base sm:text-lg leading-relaxed font-normal">
                ...
                . I don&apos;t just train models—I build production-grade
                software wrappers, clean REST APIs, and futuristic glassmorphism
                    ...
                    {item.skills && (
                      <div className="flex flex-wrap gap-2 pt-4 border-t border-white/5">
                        {item.skills.map((skill, sIdx) => (
                          <span
                            key={sIdx}
                            className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-white/5 border border-white/10 text-slate-300"
                          >
                            {skill}
                          </span>
                        ))}
